...
class AEDNavigatorOctagonSmooth(Node):
    def __init__(self):
        ...
        self.get_logger().info('=== AED Octagon Smooth Patrol Ready (No Forward after Undock) ===')
    ...
    def trigger_callback(self, msg):
        ...
        self.get_logger().info(f'>> 로봇 선택됨: {target_ns} (대기 상태)')
        
        self.is_initialized = True
        self.is_setup_in_progress = False

    def perform_undock_sequence(self):
        # [수정] Undock Action 후, 전진 동작(cmd_vel)을 완전히 제거함.
        ...
        try:
            ...
            # [삭제됨] 이전에 존재하던 '앞으로 전진' 루프를 제거했습니다.
            # 로봇은 Undock Action에 의해 충전기에서 분리된 상태로 정지합니다.
    ...
    def process_patient_mission_thread(self, msg):
        # 콜백 함수 가장 첫 실행 부분에 Undock 기능 수행
        if self.is_docked:
